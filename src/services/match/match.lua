...
-- 匹配成功
local function matchSuccess(gameid, queueid, user1, user2, config)
    ...
    matchOnSure.startOnSure(gameid, queueid, playerids,
        { rule = cjson.encode(config.rule or {}) or "", rate = { user1.rate, user2.rate } })
end

-- 匹配成功，与机器人匹配
local function matchSuccessWithRobot(gameid, queueid, user, robotData, config)
    log.info("matchWithRobot %d %s", user.userid, UTILS.tableToString(robotData))
    local playerids = { user.userid, robotData.userid }
    matchOnSure.startOnSure(gameid, queueid, playerids,
        { rule = cjson.encode(config.rule or {}) or "", robots = { robotData.userid }, rate = { user.rate, robotData.cp } })
end
...
local function checkMatchNum(gameid, queueid, user, config)
    if user.checkNum >= config.robotAfterFails then
        ...
        if robot and #robot > 0 then
            matchSuccessWithRobot(gameid, queueid, user, robot[1], config)
            return true
        end
    end
    return false
...
local function matchSuccessDynamic(gameid, queueid, users, config)
    ...
    matchOnSure.startOnSure(gameid, queueid, playerids, {
        rule = cjson.encode(config.rule or {}) or "",
        rate = rates,
    ...
    while i <= queLen do
        if i < queLen then
            ...
            if math.abs(user1.rate - user2.rate) < rateDiff then
                ...
                i = i - 1
                queLen = queLen - 2
                matchSuccess(gameid, queueid, user1, user2, config)
            else
