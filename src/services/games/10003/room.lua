...
local function addDayScore(userid, seat, rank, playerCount)
    ...
    local rankKey = "game10003DayRank:" .. day
    ...
    log.info("%s addDayScore game10003 userid=%d seat=%d rank=%d playerCount=%d score=%d totalScore=%.0f",
        roomInstance:getRoomLogTag(),
...
-- 获取本局4个数字（供AI求解使用）
function roomHandlerAi.getDealNumbers()
    if not roomInstance then return {} end
    if roomInstance.logicHandler and roomInstance.logicHandler.getDealNumbers then
        return roomInstance.logicHandler.getDealNumbers()
    end
    return {}
end
...
function Room:init(data)
    ...
    log.info("%s game10003 Room:init %s", roomInstance:getRoomLogTag(), UTILS.tableToString(data))
    ...
    if self:isMatchRoom() then
        ...
        self.roomInfo.roomWaitingConnectTime = config.MATCH_ROOM_WAITTING_CONNECT_TIME or 8
        self.roomInfo.roomGameTime = config.MATCH_ROOM_GAME_TIME or 300
        -- 初始化匹配房间玩家
...
function Room:_initMatchRoomPlayers(data)
    ...
    for seat, userid in pairs(self.roomInfo.playerids) do
        ...
        if bRobot then
            ...
            self:setUserStatus(userid, self.gConfig.USER_STATUS.GAMEING, self.roomInfo.gameid, self.roomInfo.roomid,
                self.roomInfo.addr, self.roomInfo.shortRoomid, self.roomInfo.gatewayUrl)
        end
        -- 匹配场的战力是匹配的时候传过来的
...
function Room:initLogic()
    ...
    -- 传递房间信息给子模块
    require("games.10003.scoring").setGameContext(self.roomInfo.gameid, self.roomInfo.roomid)
...
function Room:testStart()
    log.info("%s Room10003:testStart", roomInstance:getRoomLogTag())
...
function Room:startGame()
    ...
    log.info("%s game10003 game start, player count: %d", roomInstance:getRoomLogTag(), self.roomInfo.playerNum)
end

-- 重写重连方法
function Room:relink(userid)
    log.info("%s game10003 Room:relink %d", roomInstance:getRoomLogTag(), userid)
        ...
        if not alreadyIn then
            ...
            if seat then
                ...
                self:setUserStatus(userid, self.gConfig.USER_STATUS.GAMEING, self.roomInfo.gameid, self.roomInfo.roomid,
                    self.roomInfo.addr, self.roomInfo.shortRoomid, self.roomInfo.gatewayUrl)
                -- 私人房战力拉取
                local rices = skynet.call(self.svrDB, "lua", "db", "getUserRichesByType", userid,
                    CONFIG.RICH_TYPE.COMBAT_POWER)
                local cp = 0
                if rices then
                    cp = rices.richNums
                end
...
-- 提交算式
function REQUEST:submitAnswer(userid, args)
    if not roomInstance then
        return { code = 0, msg = "房间未初始化" }
    end
...
skynet.register_protocol {
    ...
    dispatch = function(fd, _, type, ...)
        log.info("%s room10003 dispatch fd %d, type %s", roomInstance:getRoomLogTag(), fd, type)
        skynet.ignoreret()
